...
| Module | Role |
|--------|------|
| `src/components/AppShell.tsx` | sticky top nav (Desk/Sectors/Compare/Jobs — Jobs hidden if API 404s), persistent debounced search (250ms) with dropdown, compare-count badge, footer disclaimer |
...
| `src/components/bars.tsx` | SVG ScoreBar (hollow on NULL), PillarMiniBars (Q·V·G·R) |
| `src/screens/Home.tsx` | Desk: status stats, signal histogram (CSS bars), "Needs history" callout, Top 10 USD + CAD + Top 10 All (score-only), add-ticker flow (ingest → recompute → dossier) |
...
| `src/screens/Sector.tsx` | currency view toggle; ALL = score-only table (both currencies) + split USD/CAD money panels; USD/CAD = classic table + money medians; checkbox → "Compare selected (2–8)" |
| `src/screens/Dossier.tsx` | identity → verdict (signal + peer rank first) → gaps → pillar board → why bullets → snapshot → history (table + SVG bars, <3 rows = callout) → similar → halal → actions (session/localStorage compare basket, "Compare with similar") |
| `src/screens/Compare.tsx` | 2–8 ids via URL, per-row pillar mini-bars, best-cell highlighting, mixed-currency warning banner |
...
| `src/lib/*` | `bars.ts` (SVG math), `allCurrency.ts` (composeAll — never blends money), `compare.ts` (max-8 builder), `nav.ts` (nav items + sector keys), `sessionCompare.ts` (localStorage `compareIds`, max 8) |
...
- Tests: backend pytest **88 passed**; frontend vitest **29 passed**; `npm run build` ✓;
...
- `frontend/src/screens/*.tsx.stale` — Phase 5 Windows-casing leftovers (lowercase
